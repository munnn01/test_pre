...
OUT="/kaggle/working/outputs/dual_v4_val/${CODEC}/shard_${SHARD}"
mkdir -p "$OUT"
finish() {
  rc=$?
  trap - EXIT
  set +e
  cd /kaggle/working
  tar -czf "dual_v4_val_${CODEC}_s${SHARD}.tgz" \
    "outputs/dual_v4_val/${CODEC}/shard_${SHARD}" 2>/dev/null
  echo "[exit] rc=$rc artifact=/kaggle/working/dual_v4_val_${CODEC}_s${SHARD}.tgz"
  exit "$rc"
}
trap finish EXIT
...
cd "$REPO"
test -s "configs/dual_codec_v4_frozen/${CODEC}/frozen_policy.json" || {
  echo 'Missing committed, frozen V4 policy' >&2; exit 2;
}
KIN_ROOT=""
...
python -m ops.dual_codec_v4_eval evaluate --archive "$ARCHIVE" \
  --v4-dir "configs/dual_codec_v4_frozen/${CODEC}" --index "$INDEX" \
...
echo "[done] codec=$CODEC shard=$SHARD V4 VAL development comparison only"
