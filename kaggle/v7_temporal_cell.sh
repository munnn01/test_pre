...
OUT="/kaggle/working/outputs/v7_temporal/${CODEC}/shard_${SHARD}"
mkdir -p "$OUT"
finish() {
  rc=$?
  trap - EXIT
  set +e
  cd /kaggle/working
  tar -czf "v7_temporal_${CODEC}_s${SHARD}.tgz" \
    "outputs/v7_temporal/${CODEC}/shard_${SHARD}" 2>/dev/null
  echo "[exit] rc=$rc artifact=/kaggle/working/v7_temporal_${CODEC}_s${SHARD}.tgz"
  exit "$rc"
}
trap finish EXIT
...
python -m ops.v7_temporal_pilot evaluate --archive "$ARCHIVE" \
...
echo "[done] codec=$CODEC shard=$SHARD V7 TRAIN feasibility only"
