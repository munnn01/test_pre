#!/usr/bin/env python
"""Push one commit-pinned V4-versus-V2 VAL-development Kaggle shard."""
...
TEMPLATE = REPO / "kaggle/v4_dual_val_cell.sh"
...
def payload(commit: str, account: str, codec: str, shard: int,
    ...
    slug = f"dual-v4-val-{codec}-s{shard}"
    ...
    book["cells"][0]["id"] = f"dual-v4-val-{codec}-s{shard}"
...
def main() -> None:
    ...
    if not (REPO / f"configs/dual_codec_v4_frozen/{args.codec}/frozen_policy.json").is_file():
        raise ValueError("V4 policy must be committed before dispatch")
