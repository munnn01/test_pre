#!/usr/bin/env python
"""Push one commit-pinned, private V5-versus-V2 VAL development shard."""
...
TEMPLATE = REPO / "kaggle/v5_repair_val_cell.sh"
...
def payload(commit: str, account: str, codec: str, shard: int,
    ...
    slug = f"dual-v5-repair-val-{codec}-s{shard}"
...
def main() -> None:
    ...
    if not (REPO / f"configs/dual_codec_v5_frozen/{args.codec}/frozen_policy.json").is_file():
        raise ValueError("V5 policy must be committed before dispatch")
